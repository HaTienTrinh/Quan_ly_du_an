...
@section('page_title', 'Bài viết đã xóa')
...
@section('content')
    <div class="d-flex flex-column flex-md-row gap-3 justify-content-between align-items-stretch align-items-md-center mb-4">
        <form method="get" action="{{ route('admin.posts.trashed') }}" class="d-flex gap-2 flex-grow-1 flex-md-grow-0" style="max-width: 420px;">
            ...
            @if(request()->filled('q'))
                <a href="{{ route('admin.posts.trashed') }}" class="btn btn-outline-secondary">Xóa bộ lọc</a>
            @endif
        </form>
        <a href="{{ route('admin.posts.index') }}" class="btn btn-outline-primary align-self-start align-self-md-center">
    ...
    <div class="alert alert-warning border-0 rounded-3 small mb-4" role="alert">
        ...
        Đây là các bài viết đã <strong>xóa mềm</strong>. Bạn có thể <strong>khôi phục</strong> hoặc <strong>xóa vĩnh viễn</strong>.
    </div>
                ...
                <tbody>
                    @forelse($posts as $post)
                        <tr>
                            ...
                            <td class="small text-muted text-nowrap">{{ $post->deleted_at?->format('d/m/Y H:i') }}</td>
                            <td class="pe-4 text-end">
                                ...
                                <form action="{{ route('admin.posts.force-destroy', $post->id) }}" method="post" class="d-inline"
                                      onsubmit="return confirm('Xóa vĩnh viễn bài viết «{{ $post->title }}»? Không thể hoàn tác.');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                ...
                                Thùng rác trống. <a href="{{ route('admin.posts.index') }}">Quay lại danh sách</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
